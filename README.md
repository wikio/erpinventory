...
IndexedDB schema version 8 now includes 55 offline stores. It adds centrally managed VAT rates, ERP-wide configurable reference masks and sequence counters, employee career records, conversations, and messages; all are included in JSON backups.
...
Version 8 adds purchase invoices/quotes/receipts, cross-document conversion and tender traceability, advanced search and catalog picking, configurable payments/banks/accounts, partner 360° views and reports, centralized GED, signed verification URLs with QR/barcodes, Algerian G50/IBS/bilan tracking, coupons/referrals, and complete employee career/attendance/performance workflows. It also provides role-filtered expandable sidebar submenus, translated CRUD master-data managers for partner/bank/product types, ISO countries, logistics statuses and Incoterms, searchable country/category/partner/product selectors, a drag-enabled sales pipeline, paginated document history, lot/batch and stock movement/count management, configurable CSV/XLSX and online-grid imports, and a token-authenticated public integration API with built-in documentation. Mask-based print references, pagination, unified discounts, visual timelines, helper/tooltips, and expanded template tools remain enforced.
