...
## 🆕 Leave lifecycle, dual e-signature & work certificates (Sections 315–322)

The employee portal now enforces the full leave lifecycle: pending requests are editable/deletable, while approved leave uses Postpone / Modify / Cancel actions (with a mandatory reason) routed for HR approval like new requests — each action with its Lucide icon. Employment contracts render as complete Algerian notarial-style documents (header/footer, pagination, identified parties, labour-code articles) with a **dual electronic signature**: both the employee and the company representative sign with name, role, place, date/time and a **canvas-drawn signature**, and the **signed PDF is archived automatically in the GED**. A configurable **job functions/tasks** list per position feeds the contract's duties article and the new **Work Certificate ("Attestation de travail")** manager (with/without salary, with tasks, manager canvas signature, employee portal requests, automatic or manual generation). The leave calendar weekend-column ordering bug is fixed for any configured weekend. See [`docs/sections-315-322.md`](docs/sections-315-322.md).

## 🆕 Payslip references, holiday sources & onboarding polish (Sections 306–314)

Payslip references now follow `SARI-PAY{YY}{MM}{PAYTYPE}{DD}-{EMPSEQ}` (payment type + employee sequence); the leave calendar uses the Algerian Sunday-first week with Friday/Saturday weekends that follow the work-schedule configuration; the public-holiday page gains CSV/JSON import with configurable mapping, server-folder auto-fetch (2011–2031 files included in `content/holidays`), a searchable year filter and automatic religious-date calculation with a per-entry editable source; payslips resolve NIF/NAI/NIS/RC per selected trade register; the GED modal stacks above the payslip consultation with a back action; the default payslip template is modernized with 3 extra designs and configurable header/footer; the onboarding stepper refreshes without reload; and detailed article-by-article Work Rules / General Terms templates (FR/AR/EN) ship as version 2. See [`docs/sections-306-314.md`](docs/sections-306-314.md).
