...
Every authenticated user can open the top-bar profile menu and choose **View My Profile** or **Update Password**. The same actions are available under **Personal Space**. Password changes use `POST /api/auth/change-password`, require the current password, enforce a different value of at least 10 characters, invalidate unused activation/reset tokens, and revoke the user's other active sessions while preserving the session that performed the change. Plaintext passwords are never persisted or returned.
