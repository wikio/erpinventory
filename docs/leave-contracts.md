...
## Section 350 follow-up — desktop icon visibility, tablet cards and sticky section tabs

Desktop now hard-hides both hamburger and scanner launch icons at ≥1024 px; scanner availability is limited to mobile/tablet widths. At tablet widths, runtime layout measurement marks only genuinely overflowing `sari-table` elements and converts them into one/two-column labeled cards, while fitting tables stay compact. Internal module navs and tab strips receive a sticky class below the measured dynamic topbar height (updated through `ResizeObserver`) so the current section remains visible during scroll.
