...
class SariApp {
  ...
  initializeSidebarMenus() {
    ...
    const groups=[
      {id:'operations',label:'Opérations',icon:'blocks',items:['inventory','inventoryOps','importExport','tenders']},
      {id:'commerce',label:'Commerce',icon:'shopping-bag',items:['sales','purchases','customers','suppliers']},
      {id:'people',label:'Équipe',icon:'users-round',items:['hr','tasks','portal']},
      {id:'analysis',label:'Analyse & Documents',icon:'chart-no-axes-combined',items:['reports','ged','taxes']},
      {id:'admin',label:'Administration',icon:'settings-2',items:['bulkImport','api','masterData','translations','auditLogs','settings']}
    ];
    const saved=JSON.parse(localStorage.getItem('sari_submenus')||'{}');
    groups.forEach(group=>{const wrapper=document.createElement('div');wrapper.className='sidebar-submenu';wrapper.dataset.menuGroup=group.id;const expanded=saved[group.id]!==false;wrapper.innerHTML=`<button class="sidebar-submenu-toggle sari-sidebar-item w-full" aria-expanded="${expanded}" title="${group.label}"><i data-lucide="${group.icon}" class="w-4 h-4"></i><span class="sari-nav-label">${group.label}</span><i data-lucide="chevron-down" class="submenu-chevron w-3 h-3"></i></button><div class="sidebar-submenu-items ${expanded?'':'collapsed'}"></div>`;const items=wrapper.querySelector('.sidebar-submenu-items');group.items.forEach(module=>{const item=root.querySelector(`[data-nav-item="${module}"]`);if(item){item.classList.add('sidebar-child-item');items.appendChild(item);}});wrapper.querySelector('.sidebar-submenu-toggle').onclick=()=>this.toggleSubmenu(group.id);root.appendChild(wrapper);});
