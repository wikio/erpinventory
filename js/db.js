...
class SariDB {
  ...
  async backfillNumericIdsAndReferences(){const stores=Array.from(this.db.objectStoreNames).filter(name=>!['recordSequences'].includes(name));for(const storeName of stores){const rows=await this.rawGetAll(storeName),seen=new Set();let next=Math.max(0,...rows.map(row=>Number(row.numericId)||0))+1;for(const row of rows){let numericId=Number(row.numericId);if(!Number.isInteger(numericId)||numericId<1||seen.has(numericId)){while(seen.has(next))next++;numericId=next++;row.numericId=numericId;}seen.add(numericId);await this.applyDerivedReference(storeName,row);await this.rawPut(storeName,row);}await this.rawPut('recordSequences',{id:storeName,value:Math.max(0,...seen),updatedAt:new Date().toISOString()});}}
