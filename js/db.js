...
class SariDB {
    ...
    this.initPromise = new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        const stores = [
          ...
          'leaveTypes', 'publicHolidays', 'leaveRequests', 'workedHolidays', 'paymentTypes', 'employmentContracts', 'ruleAcceptances', 'conflictDeclarations', 'workRules', 'occasionalWorkers', 'workerAssignments', 'onboardingStates', 'jobFunctions', 'workCertificates'
        ];
        ...
        stores.forEach((storeName) => {
          if (!db.objectStoreNames.contains(storeName)) {
            ...
            } else if (storeName === 'leaveRequests' || storeName === 'ruleAcceptances' || storeName === 'conflictDeclarations' || storeName === 'onboardingStates' || storeName === 'employmentContracts') {
              store.createIndex('employeeId', 'employeeId', { unique: false });
            ...
            } else if (storeName === 'workCertificates') {
              store.createIndex('employeeId', 'employeeId', { unique: false });
      ...
      request.onsuccess = async (event) => {
        ...
        await this.ensureEnhancement300Data();
        await this.ensureEnhancement315Data();
  ...
  async ensureEnhancement300Data(){
    // Section 300–305 reference codes (leave, contracts, occasional workers)
    const codes300=[['CONG','Demande de congé','طلب إجازة','Leave request'],['CTT','Contrat de travail','عقد عمل','Employment contract'],['OCC','Travailleur occasionnel','عامل عرضي','Occasional worker']];
    for(const[code,fr,ar,en]of codes300)if(!await this.getById('documentCodes',code))await this.save('documentCodes',{id:code,code,designation:fr,designationI18n:{fr,ar,en},description:en,mask:`SARI-${code}{YY}-{SEQ}`,example:`SARI-${code}26-00001`,maskType:'Standard',sequenceMinDigits:5,resetFrequency:'yearly',isActive:true,subTypeOptions:[]});
    // GED integration
    const ged300=[['leaveRequest','Congés & absences','الإجازات والغيابات','Leave & absences'],['employmentContract','Contrats de travail','عقود العمل','Employment contracts'],['workerAssignment','Missions occasionnelles','مهام عرضية','Occasional assignments']];
    for(const[id,fr,ar,en]of ged300)if(!await this.getById('gedModules',id))await this.save('gedModules',{id,code:id,name:{fr,ar,en},order:30,isActive:true});
    for(const id of['leaveRequest','employmentContract','workerAssignment'])if(!await this.getById('gedTypes',id))await this.save('gedTypes',{id,code:id,name:{fr:{leaveRequest:'Demande de congé',employmentContract:'Contrat de travail',workerAssignment:'Mission occasionnelle'}[id],ar:{leaveRequest:'طلب إجازة',employmentContract:'عقد عمل',workerAssignment:'مهمة عرضية'}[id],en:{leaveRequest:'Leave request',employmentContract:'Employment contract',workerAssignment:'Occasional assignment'}[id]},order:30,isActive:true});
    // 300.1 Leave types
    const leaveTypes=[
      {id:'lt-annual',code:'annual',name:{fr:'Congé annuel payé',ar:'إجازة سنوية مدفوعة',en:'Annual paid leave'},daysPerYear:30,isPaid:true,deductsBalance:true,requiresJustification:false,maxDaysPerRequest:30,color:'#009CC5',order:1,isActive:true},
      {id:'lt-sick',code:'sick',name:{fr:'Congé maladie',ar:'إجازة مرضية',en:'Sick leave'},daysPerYear:15,isPaid:true,deductsBalance:true,requiresJustification:true,maxDaysPerRequest:15,color:'#EBB51A',order:2,isActive:true},
      {id:'lt-unpaid',code:'unpaid',name:{fr:'Congé sans solde',ar:'إجازة بدون أجر',en:'Unpaid leave'},daysPerYear:0,isPaid:false,deductsBalance:false,requiresJustification:true,maxDaysPerRequest:90,color:'#EF4444',order:3,isActive:true},
      {id:'lt-maternity',code:'maternity',name:{fr:'Congé de maternité (14 semaines)',ar:'إجازة أمومة (14 أسبوعًا)',en:'Maternity leave (14 weeks)'},daysPerYear:98,isPaid:true,deductsBalance:false,requiresJustification:true,maxDaysPerRequest:98,color:'#C6DA34',order:4,isActive:true},
      {id:'lt-marriage',code:'marriage',name:{fr:'Congé de mariage',ar:'إجازة زواج',en:'Marriage leave'},daysPerYear:3,isPaid:true,deductsBalance:true,requiresJustification:false,maxDaysPerRequest:3,color:'#8B5CF6',order:5,isActive:true},
      {id:'lt-compassionate',code:'compassionate',name:{fr:'Congé exceptionnel (décès / événement familial)',ar:'إجازة استثنائية (وفاة / حدث عائلي)',en:'Compassionate leave'},daysPerYear:3,isPaid:true,deductsBalance:false,requiresJustification:true,maxDaysPerRequest:3,color:'#64748B',order:6,isActive:true}
    ];
    for(const type of leaveTypes)if(!await this.getById('leaveTypes',type.id))await this.save('leaveTypes',type);
    // 301 Algerian public holiday calendar (fixed dates + variable religious dates for 2026)
    const holidays=[
      {id:'ph-2026-newyear',date:'2026-01-01',name:{fr:'Jour de l’an',ar:'رأس السنة الميلادية',en:'New Year’s Day'},isFixed:true,order:1,baseKey:'new_year',source:'auto'},
      {id:'ph-2026-yennayer',date:'2026-01-12',name:{fr:'Yennayer (Nouvel an amazigh)',ar:'يناير (رأس السنة الأمازيغية)',en:'Yennayer (Amazigh New Year)'},isFixed:true,order:2,baseKey:'yennayer',source:'auto'},
      {id:'ph-2026-eid-fitr-1',date:'2026-03-20',name:{fr:'Aïd el-Fitr (1er jour)',ar:'عيد الفطر (اليوم الأول)',en:'Eid al-Fitr (1st day)'},isFixed:false,order:3,baseKey:'eid_fitr_1',source:'auto'},
      {id:'ph-2026-eid-fitr-2',date:'2026-03-21',name:{fr:'Aïd el-Fitr (2e jour)',ar:'عيد الفطر (اليوم الثاني)',en:'Eid al-Fitr (2nd day)'},isFixed:false,order:4,baseKey:'eid_fitr_2',source:'auto'},
      {id:'ph-2026-labour',date:'2026-05-01',name:{fr:'Fête du Travail',ar:'عيد العمال',en:'Labour Day'},isFixed:true,order:5,baseKey:'labour_day',source:'auto'},
      {id:'ph-2026-eid-adha-1',date:'2026-05-27',name:{fr:'Aïd el-Adha (1er jour)',ar:'عيد الأضحى (اليوم الأول)',en:'Eid al-Adha (1st day)'},isFixed:false,order:6,baseKey:'eid_adha_1',source:'auto'},
      {id:'ph-2026-eid-adha-2',date:'2026-05-28',name:{fr:'Aïd el-Adha (2e jour)',ar:'عيد الأضحى (اليوم الثاني)',en:'Eid al-Adha (2nd day)'},isFixed:false,order:7,baseKey:'eid_adha_2',source:'auto'},
      {id:'ph-2026-muharram',date:'2026-06-16',name:{fr:'Awal Moharram (Nouvel an hégirien)',ar:'أول محرم (رأس السنة الهجرية)',en:'Awal Muharram (Islamic New Year)'},isFixed:false,order:8,baseKey:'awal_muharram',source:'auto'},
      {id:'ph-2026-ashura',date:'2026-06-25',name:{fr:'Achoura',ar:'عاشوراء',en:'Ashura'},isFixed:false,order:9,baseKey:'ashura',source:'auto'},
      {id:'ph-2026-independence',date:'2026-07-05',name:{fr:'Fête de l’Indépendance',ar:'عيد الاستقلال',en:'Independence Day'},isFixed:true,order:10,baseKey:'independence_day',source:'auto'},
      {id:'ph-2026-mawlid',date:'2026-08-26',name:{fr:'Mawlid Ennabaoui',ar:'المولد النبوي الشريف',en:'Mawlid Ennabaoui'},isFixed:false,order:11,baseKey:'mawlid',source:'auto'},
      {id:'ph-2026-revolution',date:'2026-11-01',name:{fr:'Fête de la Révolution (1er novembre)',ar:'عيد الثورة (1 نوفمبر)',en:'Revolution Day (November 1st)'},isFixed:true,order:12,baseKey:'revolution_day',source:'auto'}
    ];
    for(const holiday of holidays){const existing=await this.getById('publicHolidays',holiday.id);if(existing){if(!existing.baseKey||!existing.source||!existing.year){existing.baseKey=holiday.baseKey;existing.source=existing.source||'auto';existing.year=2026;await this.rawPut('publicHolidays',existing);}}else await this.save('publicHolidays',{...holiday,year:2026,isWorkable:true,notes:holiday.isFixed?'':'Date religieuse variable : ajustable selon l’annonce officielle (calcul automatique disponible).'});}
    // 300.4 Payment types (with Algerian pigiste rules)
    const paymentTypes=[
      {id:'pt-daily',code:'daily',name:{fr:'Journalier',ar:'يومي',en:'Daily'},cycle:'daily',isPiecework:false,cnasApplicable:true,irgApplicable:true,irgWithholdingRate:null,legalNotesI18n:{fr:'Rémunération calculée par journée de travail ; déclarée CNAS et soumise à l’IRG selon le barème salarial.',ar:'أجر يُحسب عن كل يوم عمل؛ يُصرح به لدى CNAS ويخضع لـ IRG وفق سلم الرواتب.',en:'Pay computed per working day; declared under CNAS and subject to IRG under the salary scale.'},order:1,isActive:true},
      {id:'pt-weekly',code:'weekly',name:{fr:'Hebdomadaire',ar:'أسبوعي',en:'Weekly'},cycle:'weekly',isPiecework:false,cnasApplicable:true,irgApplicable:true,irgWithholdingRate:null,legalNotesI18n:{fr:'Rémunération hebdomadaire dans le cadre d’un contrat salarié (CNAS + IRG barème).',ar:'أجر أسبوعي ضمن عقد عمل (CNAS + IRG حسب السلم).',en:'Weekly pay under a salaried contract (CNAS + IRG scale).'},order:2,isActive:true},
      {id:'pt-monthly',code:'monthly',name:{fr:'Mensuel',ar:'شهري',en:'Monthly'},cycle:'monthly',isPiecework:false,cnasApplicable:true,irgApplicable:true,irgWithholdingRate:null,legalNotesI18n:{fr:'Salaire mensuel standard : cotisations CNAS salarié 9 % / employeur 26 % et retenue IRG progressive.',ar:'الراتب الشهري القياسي: اشتراكات CNAS العامل 9% / رب العمل 26% واقتطاع IRG تصاعدي.',en:'Standard monthly salary: CNAS 9% employee / 26% employer contributions and progressive IRG withholding.'},order:3,isActive:true},
      {id:'pt-annual',code:'annual',name:{fr:'Annuel',ar:'سنوي',en:'Annual'},cycle:'annual',isPiecework:false,cnasApplicable:true,irgApplicable:true,irgWithholdingRate:null,legalNotesI18n:{fr:'Rémunération annuelle contractuelle (dirigeants ou conventions spécifiques), versée par échéances.',ar:'أجر سنوي تعاقدي (مسيّرون أو اتفاقيات خاصة) يُدفع على أقساط.',en:'Contractual annual remuneration (executives or specific agreements), paid in instalments.'},order:4,isActive:true},
      {id:'pt-piecework',code:'piecework',name:{fr:'Occasionnel / Pigiste (tâche)',ar:'عرضي / بالمهمة (Pigiste)',en:'Occasional / Piece-work (Pigiste)'},cycle:'piecework',isPiecework:true,cnasApplicable:false,irgApplicable:true,irgWithholdingRate:0.15,legalNotesI18n:{fr:'Personne rémunérée pour une mission précise (pigiste / tâcheron) : NON déclarée CNAS. Retenue à la source IRG sur le montant brut (15 % par défaut, taux configurable), justificatif contractuel ou facture requis, aucune indemnité de congés payés ni couverture sociale salariée. Régime distinct du salariat classique.',ar:'شخص يُكافأ عن مهمة محددة (بالقطعة / متعاقد عرضي): غير مُصرح به لدى CNAS. اقتطاع IRG من المصدر على المبلغ الإجمالي (15% افتراضيًا، نسبة قابلة للتعديل)، مع اشتراط عقد أو فاتورة، وبدون تعويض عطلة مدفوعة أو تغطية اجتماعية للعمال. نظام مختلف عن الأجر الكلاسيكي.',en:'Person paid for a specific job (piece-work / freelancer): NOT declared under CNAS. IRG withheld at source on the gross amount (15% default, configurable), contract or invoice justification required, no paid-leave indemnity and no salaried social coverage. A regime distinct from regular salaried employment.'},order:5,isActive:true}
    ];
    for(const type of paymentTypes)if(!await this.getById('paymentTypes',type.id))await this.save('paymentTypes',type);
    // 304.2 / 314 Work rules & general terms (detailed article-by-article templates, version 2)
    const policies=globalThis.SariCore?.hrPolicies;const rulesLatest=(await this.getAll('workRules')).filter(rule=>rule.kind==='rules').sort((a,b)=>Number(b.version)-Number(a.version))[0],termsLatest=(await this.getAll('workRules')).filter(rule=>rule.kind==='terms').sort((a,b)=>Number(b.version)-Number(a.version))[0];if(!rulesLatest||Number(rulesLatest.version)<2)await this.save('workRules',{id:`rules-v2-${crypto.randomUUID()}`,kind:'rules',version:2,titleI18n:policies?policies.workRulesPolicy.title:{fr:'Règlement intérieur SARI SYSTÈME',ar:'النظام الداخلي SARI SYSTÈME',en:'SARI SYSTÈME Internal Work Rules'},contentI18n:{fr:policies?policies.policyHtml(policies.workRulesPolicy,'fr'):'<h2>Règlement intérieur — SARI SYSTÈME</h2>',ar:policies?policies.policyHtml(policies.workRulesPolicy,'ar'):'<h2>النظام الداخلي</h2>',en:policies?policies.policyHtml(policies.workRulesPolicy,'en'):'<h2>Internal Work Rules</h2>'},publishedAt:new Date().toISOString()});if(!termsLatest||Number(termsLatest.version)<2)await this.save('workRules',{id:`terms-v2-${crypto.randomUUID()}`,kind:'terms',version:2,titleI18n:policies?policies.generalTermsPolicy.title:{fr:'Conditions générales d’utilisation des outils SARI',ar:'الشروط العامة لاستخدام أدوات SARI',en:'General Terms of Use for SARI Tools'},contentI18n:{fr:policies?policies.policyHtml(policies.generalTermsPolicy,'fr'):'<h2>Conditions générales d’utilisation</h2>',ar:policies?policies.policyHtml(policies.generalTermsPolicy,'ar'):'<h2>الشروط العامة</h2>',en:policies?policies.policyHtml(policies.generalTermsPolicy,'en'):'<h2>General terms of use</h2>'},publishedAt:new Date().toISOString()});
// 304.1 Employment contracts for the demo staff (two signed, one awaiting signature)
    if(!await this.getById('employmentContracts','ctt-emp-admin'))await this.save('employmentContracts',{id:'ctt-emp-admin',employeeId:'emp-admin',employeeName:'Amel Bensaïd',type:'CDI',title:'Contrat de travail à durée indéterminée — Directrice générale',position:'Directrice générale',department:'Direction',startDate:'2021-03-01',endDate:'',trialPeriodMonths:3,baseSalary:180000,weeklyHours:40,paymentTypeId:'pt-monthly',clausesHtml:'<p>Le salarié exerce ses fonctions conformément au règlement intérieur.</p>',status:'signed',sentAt:'2021-03-01',signature:{name:'Amel Bensaïd',userId:'usr-admin',signedAt:'2021-03-01',device:'SARI Onboarding'}});
    if(!await this.getById('employmentContracts','ctt-emp-stock'))await this.save('employmentContracts',{id:'ctt-emp-stock',employeeId:'emp-stock',employeeName:'Nadir Khelifi',type:'CDI',title:'Contrat de travail à durée indéterminée — Gestionnaire stocks',position:'Gestionnaire stocks',department:'Logistique',startDate:'2023-06-12',endDate:'',trialPeriodMonths:3,baseSalary:95000,weeklyHours:40,paymentTypeId:'pt-monthly',clausesHtml:'<p>Respect des procédures de traçabilité des lots et de la chaîne du froid.</p>',status:'signed',sentAt:'2023-06-12',signature:{name:'Nadir Khelifi',userId:'usr-stock',signedAt:'2023-06-12',device:'SARI Onboarding'}});
    if(!await this.getById('employmentContracts','ctt-emp-sales'))await this.save('employmentContracts',{id:'ctt-emp-sales',employeeId:'emp-sales',employeeName:'Lina Mansouri',type:'CDI',title:'Contrat de travail à durée indéterminée — Commerciale B2B',position:'Commerciale B2B',department:'Commercial',startDate:'2024-01-15',endDate:'',trialPeriodMonths:3,baseSalary:105000,weeklyHours:40,paymentTypeId:'pt-monthly',clausesHtml:'<p>Objectifs commerciaux annuels et confidentialité du portefeuille clients.</p>',status:'sent',sentAt:'2024-01-15'});
    // 304.3 Conflict-of-interest declaration (admin already completed, others pending)
    if(!await this.getById('conflictDeclarations','cd-emp-admin'))await this.save('conflictDeclarations',{id:'cd-emp-admin',employeeId:'emp-admin',ethicsCommitment:true,confidentiality:true,noConflicts:true,hasExternalEngagements:false,externalEngagementsHtml:'',declaredAt:'2026-01-10',updatedAt:'2026-01-10'});
    // 304.4 Onboarding snapshot (rules read & accepted by admin only)
    if(!await this.getById('ruleAcceptances','ra-admin-rules'))await this.save('ruleAcceptances',{id:'ra-admin-rules',employeeId:'emp-admin',kind:'rules',version:1,acceptedAt:'2026-01-10',signature:{name:'Amel Bensaïd',userId:'usr-admin',signedAt:'2026-01-10',device:'SARI Onboarding'}});
    if(!await this.getById('ruleAcceptances','ra-admin-terms'))await this.save('ruleAcceptances',{id:'ra-admin-terms',employeeId:'emp-admin',kind:'terms',version:1,acceptedAt:'2026-01-10',signature:{name:'Amel Bensaïd',userId:'usr-admin',signedAt:'2026-01-10',device:'SARI Onboarding'}});
    // 300.2 / 302 Demo leave requests (paid approved + unpaid submitted for the auto-adjustment flow)
    if(!await this.getById('leaveRequests','lv-emp-sales-annual'))await this.save('leaveRequests',{id:'lv-emp-sales-annual',employeeId:'emp-sales',employeeName:'Lina Mansouri',department:'Commercial',typeId:'lt-annual',isPaid:true,startDate:'2026-09-02',endDate:'2026-09-06',durationDays:3,status:'approved',reason:'Repos estival',requestedAt:'2026-08-02',decidedBy:'emp-admin',decidedAt:'2026-08-03'});
    if(!await this.getById('leaveRequests','lv-emp-stock-unpaid'))await this.save('leaveRequests',{id:'lv-emp-stock-unpaid',employeeId:'emp-stock',employeeName:'Nadir Khelifi',department:'Logistique',typeId:'lt-unpaid',isPaid:false,startDate:'2026-08-24',endDate:'2026-08-26',durationDays:3,status:'approved',reason:'Démarches personnelles',requestedAt:'2026-08-10',decidedBy:'emp-admin',decidedAt:'2026-08-11'});
    // 303 Temporary / occasional worker demo
    if(!await this.getById('occasionalWorkers','occ-demo'))await this.save('occasionalWorkers',{id:'occ-demo',firstName:'Karim',lastName:'Bouzid',cin:'1098765432',phone:'+213 555 12 34 56',email:'karim.bouzid@example.dz',skill:'Inventaire physique dépôt',rate:3500,paymentTypeId:'pt-piecework',status:'active',hiredAt:'2026-08-10',notes:'Pigiste — inventaire annuel du dépôt central.'});
    ...
    const tplDesigns=[
      ...
      {id:'doc-tpl-payslip-clinical',name:'Fiche de paie clinique',nameI18n:{fr:'Fiche de paie clinique',ar:'قسيمة راتب سريرية',en:'Clinical payslip'},accent:'#0D9488',isDefault:false,band:'#0D9488',bandText:'#ffffff'},
      {id:'doc-tpl-payslip-amber',name:'Fiche de paie ambre',nameI18n:{fr:'Fiche de paie ambre',ar:'قسيمة راتب كهرمانية',en:'Amber payslip'},accent:'#EBB51A',isDefault:false,band:'#EBB51A',bandText:'#0f172a'},
      {id:'doc-tpl-payslip-midnight',name:'Fiche de paie nuit',nameI18n:{fr:'Fiche de paie nuit',ar:'قسيمة راتب ليلية',en:'Midnight payslip'},accent:'#38BDF8',isDefault:false,band:'#0F172A',bandText:'#38BDF8'}
    ];
    for(const design of tplDesigns){if(!await this.getById('documentTemplates',design.id))await this.save('documentTemplates',{id:design.id,name:design.name,nameI18n:design.nameI18n,type:'payslip',paperFormat:'A4',accent:design.accent,layout:'designer',templateMode:'designer',fontFamily:'Plus Jakarta Sans',isDefault:design.isDefault,headerHtml:header306,footerHtml:footer306,lineItemsColumns:[],versions:[],elements:[
      {id:'logo',kind:'field',field:'documentLogo',content:'Logo',x:35,y:22,w:130,h:54},
      {id:'company',kind:'field',field:'companyHeader',content:'Entreprise',x:180,y:22,w:380,h:58,color:'#0f172a'},
      {id:'band',kind:'static',content:'<div style="background:'+design.band+';color:'+design.bandText+';padding:14px 22px;border-radius:8px"><b style="font-size:20px">FICHE DE PAIE</b></div>',x:35,y:92,w:724,h:52},
      {id:'employee',kind:'field',field:'employeeIdentity',content:'Salarié',x:35,y:160,w:350,h:88,background:'#f8fafc',borderColor:'#cbd5e1'},
      {id:'period',kind:'field',field:'payPeriod',content:'Période',x:400,y:160,w:180,h:44,borderColor:'#cbd5e1'},
      {id:'work',kind:'field',field:'workTime',content:'Jours / heures',x:595,y:160,w:164,h:44,borderColor:'#cbd5e1'},
      {id:'earnings',kind:'field',field:'earningsTable',content:'Gains',x:35,y:268,w:350,h:300},
      {id:'deductions',kind:'field',field:'deductionsTable',content:'Retenues',x:409,y:268,w:350,h:300},
      {id:'cnas',kind:'field',field:'cnasBreakdown',content:'CNAS',x:35,y:588,w:350,h:90,background:'#f8fafc',borderColor:'#cbd5e1'},
      {id:'net',kind:'field',field:'netPayable',content:'NET À PAYER',x:409,y:588,w:350,h:90,background:design.accent+'1a',borderColor:design.accent},
    ...
    // Grant view-only access on the new HR screens to observers so the calendar stays consultable.
    const roles=await this.getAll('roles');for(const role of roles){if(role.id==='admin')continue;const additions=role.id==='readonly'?['leaves.view','contracts.view','occasionalWorkers.view']:['leaves.view'];role.permissions=[...new Set([...(role.permissions||[]),...additions])];await this.save('roles',role);}
  }
  async ensureEnhancement315Data(){
    // 319 — configurable job functions/tasks per position (reused by contracts and work certificates).
    const jobFunctions=[
      {id:'jf-direction',position:{fr:'Directrice générale',ar:'مديرة عامة',en:'Managing Director'},tasks:{fr:'<ul><li>Définir et piloter la stratégie commerciale et logistique de la société.</li><li>Superviser les achats, les ventes et les appels d’offres institutionnels.</li><li>Représenter la société auprès des administrations (CNAS, commerce, fisc) et des partenaires.</li><li>Valider les engagements financiers et les recrutements.</li><li>Garantir la conformité réglementaire des dispositifs médicaux distribués.</li></ul>',ar:'<ul dir="rtl"><li>تحديد وقيادة الاستراتيجية التجارية واللوجستية للشركة.</li><li>الإشراف على المشتريات والمبيعات والمناقصات المؤسساتية.</li><li>تمثيل الشركة لدى الإدارات (CNAS، التجارة، الضرائب) والشركاء.</li><li>المصادقة على الالتزامات المالية والتوظيف.</li><li>ضمان المطابقة التنظيمية للأجهزة الطبية الموزعة.</li></ul>',en:'<ul><li>Define and drive the company’s commercial and logistics strategy.</li><li>Supervise purchasing, sales and institutional tenders.</li><li>Represent the company before administrations (CNAS, trade, tax) and partners.</li><li>Approve financial commitments and recruitments.</li><li>Ensure the regulatory compliance of distributed medical devices.</li></ul>'},order:1,isActive:true},
      {id:'jf-stock',position:{fr:'Gestionnaire stocks',ar:'مسير المخزون',en:'Warehouse manager'},tasks:{fr:'<ul><li>Réceptionner, contrôler et ranger les dispositifs médicaux et consommables.</li><li>Tenir à jour la traçabilité des lots, des dates de péremption et de la chaîne du froid.</li><li>Préparer les expéditions et les bons de livraison.</li><li>Participer aux inventaires physiques et signaler les écarts.</li><li>Veiller au respect des règles d’hygiène et de sécurité de l’entrepôt.</li></ul>',ar:'<ul dir="rtl"><li>استلام ومراقبة وترتيب الأجهزة والمستهلكات الطبية.</li><li>تحديث تتبع الحصص وتواريخ الصلاحية وسلسلة التبريد.</li><li>تحضير الشحنات ووصلات التسليم.</li><li>المشاركة في الجرد الفعلي والإبلاغ عن الفروقات.</li><li>احترام قواعد النظافة والسلامة في المستودع.</li></ul>',en:'<ul><li>Receive, check and store medical devices and consumables.</li><li>Keep lot traceability, expiry dates and the cold chain up to date.</li><li>Prepare shipments and delivery notes.</li><li>Take part in physical inventories and report discrepancies.</li><li>Comply with warehouse hygiene and safety rules.</li></ul>'},order:2,isActive:true},
      {id:'jf-sales',position:{fr:'Commerciale B2B',ar:'تجارية B2B',en:'B2B Sales representative'},tasks:{fr:'<ul><li>Prospecter et fidéliser les hôpitaux, cliniques et pharmacies.</li><li>Élaborer les devis, commandes et dossiers d’appels d’offres.</li><li>Suivre les créances clients et relancer les règlements.</li><li>Remonter les besoins du marché et les informations concurrentielles.</li><li>Tenir à jour le portefeuille clients dans le module commercial.</li></ul>',ar:'<ul dir="rtl"><li>استقطاب واستبقاء المستشفيات والعيادات والصيدليات.</li><li>إعداد عروض الأسعار والطلبيات وملفات المناقصات.</li><li>متابعة ديون العملاء والمطالبة بالتسديد.</li><li>رصد احتياجات السوق والمعلومات التنافسية.</li><li>تحديث محفظة العملاء في الوحدة التجارية.</li></ul>',en:'<ul><li>Prospect and retain hospitals, clinics and pharmacies.</li><li>Prepare quotes, orders and tender files.</li><li>Follow client receivables and chase payments.</li><li>Report market needs and competitive information.</li><li>Keep the client portfolio up to date in the sales module.</li></ul>'},order:3,isActive:true},
      {id:'jf-quality',position:{fr:'Responsable qualité',ar:'مسؤول الجودة',en:'Quality manager'},tasks:{fr:'<ul><li>Piloter le système de management de la qualité (ISO 13485, MSPRH).</li><li>Contrôler les certifications CE et les dossiers techniques des produits.</li><li>Gérer les non-conformités, les réclamations et les retraits de lots.</li><li>Organiser les audits internes et la formation du personnel.</li><li>Assurer la veille réglementaire du secteur des dispositifs médicaux.</li></ul>',ar:'<ul dir="rtl"><li>قيادة نظام إدارة الجودة (ISO 13485، MSPRH).</li><li>مراقبة شهادات CE والملفات التقنية للمنتجات.</li><li>إدارة حالات عدم المطابقة والشكاوى وسحب الحصص.</li><li>تنظيم التدقيقات الداخلية وتكوين الموظفين.</li><li>متابعة الرقابة التنظيمية لقطاع الأجهزة الطبية.</li></ul>',en:'<ul><li>Drive the quality management system (ISO 13485, MSPRH).</li><li>Check CE certifications and product technical files.</li><li>Manage non-conformities, complaints and lot recalls.</li><li>Organise internal audits and staff training.</li><li>Monitor the regulatory watch of the medical-device sector.</li></ul>'},order:4,isActive:true}
    ];
    for(const item of jobFunctions)if(!await this.getById('jobFunctions',item.id))await this.save('jobFunctions',item);
    // 321 — configurable work-certificate types.
    const certificateTypes=[['withSalary','Avec informations de salaire','مع معلومات الأجر','With salary information'],['withoutSalary','Sans informations de salaire','بدون معلومات الأجر','Without salary information'],['withTasks','Avec fonctions & tâches du poste','مع مهام ووظائف المنصب','With job functions & tasks']];
    for(const[value,fr,ar,en]of certificateTypes){const id=`workCertificateType-${value}`;if(!await this.getById('configurableOptions',id))await this.save('configurableOptions',{id,listKey:'workCertificateType',value,name:{fr,ar,en},order:certificateTypes.findIndex(item=>item[0]===value)+1,isActive:true});}
    // GED integration for certificates.
    if(!await this.getById('gedModules','workCertificate'))await this.save('gedModules',{id:'workCertificate',code:'workCertificate',name:{fr:'Attestations de travail',ar:'شهادات العمل',en:'Work certificates'},order:31,isActive:true});
    if(!await this.getById('gedTypes','workCertificate'))await this.save('gedTypes',{id:'workCertificate',code:'workCertificate',name:{fr:'Attestation de travail',ar:'شهادة عمل',en:'Work certificate'},order:31,isActive:true});
    // 321 — generation policy: automatic or manual.
    if(!await this.getById('settings','work-certificate-config'))await this.save('settings',{id:'work-certificate-config',autoGenerate:false,updatedAt:new Date().toISOString()});
  }
  async ensureSariReferencePrefixes(){const labels={FAV:{fr:'Facture vente',ar:'فاتورة بيع',en:'Sales invoice'},FAC:{fr:'Facture achat',ar:'فاتورة شراء',en:'Purchase invoice'},DVV:{fr:'Devis vente',ar:'عرض سعر بيع',en:'Sales quote'},DVA:{fr:'Devis achat',ar:'عرض سعر شراء',en:'Purchase quote'},BCV:{fr:'Commande vente',ar:'طلبية بيع',en:'Sales order'},BCA:{fr:'Commande achat',ar:'طلبية شراء',en:'Purchase order'},LIV:{fr:'Bon de livraison',ar:'وصل تسليم',en:'Delivery note'},REC:{fr:'Bon de réception',ar:'وصل استلام',en:'Goods receipt'}};if(!await this.getById('documentCodes','JOB'))await this.save('documentCodes',{id:'JOB',code:'JOB',designation:'Offre d’emploi',designationI18n:{fr:'Offre d’emploi',ar:'عرض عمل',en:'Job posting'},description:'Recruitment job posting',mask:'SARI-JOB{YY}-{SEQ}',example:'SARI-JOB26-00001',maskType:'Standard',sequenceMinDigits:5,resetFrequency:'yearly',isActive:true,subTypeOptions:[]});const complianceCodes=[['CNS','Déclaration CNAS','تصريح CNAS','CNAS declaration'],['CSN','Déclaration CASNOS','تصريح CASNOS','CASNOS declaration'],['ACT','Actionnaire','مساهم','Shareholder'],['RGC','Registre société','سجل الشركة','Company register'],['SOC','Compte social','حساب اجتماعي','Social account'],['REQ','Demande commerce','طلب تجاري','Trade request'],['PAY','Fiche de paie','قسيمة الراتب','Payslip'],['ATT','Attestation de travail','شهادة عمل','Work certificate']];for(const[code,fr,ar,en]of complianceCodes)if(!await this.getById('documentCodes',code))await this.save('documentCodes',{id:code,code,designation:fr,designationI18n:{fr,ar,en},description:en,mask:`SARI-${code}{YY}-{SEQ}`,example:`SARI-${code}26-00001`,maskType:'Standard',sequenceMinDigits:5,resetFrequency:'yearly',isActive:true,subTypeOptions:[]});const pay=await this.getById('documentCodes','PAY');if(pay){pay.mask='SARI-PAY{YY}{MM}{PAYTYPE}{DD}-{EMPSEQ}';pay.example='SARI-PAY2603MO05-00042';pay.description='Section 306 : année, mois, type de paiement, jour de paiement et séquence employé.';pay.maskType='Standard';pay.resetFrequency='monthly';await this.rawPut('documentCodes',pay);}for(const definition of await this.getAll('documentCodes')){if(labels[definition.code])definition.designationI18n={...(definition.designationI18n||{}),...labels[definition.code]};if(!String(definition.mask||'').startsWith('SARI-')){definition.mask=`SARI-${definition.mask}`;definition.example=definition.example?.startsWith('SARI-')?definition.example:`SARI-${definition.example||definition.code}`;}await this.rawPut('documentCodes',definition);}}
