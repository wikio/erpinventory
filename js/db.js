...
class SariDB {
  constructor(dbName = 'SariSystemeDB', version = 11) {
    this.dbName = dbName;
    ...
    this.initPromise = new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        const stores = [
          ...
          'entityTranslations', 'gedCategories', 'gedModules', 'gedTags', 'configurableOptions'
        ];
      ...
      request.onsuccess = async (event) => {
        ...
        if (await this.count('gedCategories') === 0 || !await this.getById('configurableOptions','documentStatus-quoted')) await this.seedTranslationData();
        resolve(this.db);
      };
  ...
  async exportJSON() {
    ...
    const stores = [
      ...
      'productLots', 'stockMovements', 'inventoryCounts', 'importProfiles', 'apiTokens', 'apiEndpoints', 'logisticsStatuses', 'incoterms', 'paymentTransactions', 'reportAnnotations', 'entityTranslations', 'gedCategories', 'gedModules', 'gedTags', 'configurableOptions'
    ];
    const exportData = {
  ...
  async seedTranslationData() {
    ...
    for(const row of categories)await this.save('gedCategories',row);for(const row of modules)await this.save('gedModules',row);for(const row of tags)await this.save('gedTags',row);
