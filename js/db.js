...
class SariDB {
  ...
  async ensureEnhancement300Data(){
    ...
    const holidays=[
      {id:'ph-2026-newyear',date:'2026-01-01',name:{fr:'Jour de l’an',ar:'رأس السنة الميلادية',en:'New Year’s Day'},isFixed:true,order:1},
      {id:'ph-2026-yennayer',date:'2026-01-12',name:{fr:'Yennayer (Nouvel an amazigh)',ar:'يناير (رأس السنة الأمازيغية)',en:'Yennayer (Amazigh New Year)'},isFixed:true,order:2},
      {id:'ph-2026-eid-fitr-1',date:'2026-03-20',name:{fr:'Aïd el-Fitr (1er jour)',ar:'عيد الفطر (اليوم الأول)',en:'Eid al-Fitr (1st day)'},isFixed:false,order:3},
      {id:'ph-2026-eid-fitr-2',date:'2026-03-21',name:{fr:'Aïd el-Fitr (2e jour)',ar:'عيد الفطر (اليوم الثاني)',en:'Eid al-Fitr (2nd day)'},isFixed:false,order:4},
      {id:'ph-2026-labour',date:'2026-05-01',name:{fr:'Fête du Travail',ar:'عيد العمال',en:'Labour Day'},isFixed:true,order:5},
      {id:'ph-2026-eid-adha-1',date:'2026-05-27',name:{fr:'Aïd el-Adha (1er jour)',ar:'عيد الأضحى (اليوم الأول)',en:'Eid al-Adha (1st day)'},isFixed:false,order:6},
      {id:'ph-2026-eid-adha-2',date:'2026-05-28',name:{fr:'Aïd el-Adha (2e jour)',ar:'عيد الأضحى (اليوم الثاني)',en:'Eid al-Adha (2nd day)'},isFixed:false,order:7},
      {id:'ph-2026-muharram',date:'2026-06-16',name:{fr:'Awal Moharram (Nouvel an hégirien)',ar:'أول محرم (رأس السنة الهجرية)',en:'Awal Muharram (Islamic New Year)'},isFixed:false,order:8},
      {id:'ph-2026-ashura',date:'2026-06-25',name:{fr:'Achoura',ar:'عاشوراء',en:'Ashura'},isFixed:false,order:9},
      {id:'ph-2026-independence',date:'2026-07-05',name:{fr:'Fête de l’Indépendance',ar:'عيد الاستقلال',en:'Independence Day'},isFixed:true,order:10},
      {id:'ph-2026-mawlid',date:'2026-08-26',name:{fr:'Mawlid Ennabaoui',ar:'المولد النبوي الشريف',en:'Mawlid Ennabaoui'},isFixed:false,order:11},
      {id:'ph-2026-revolution',date:'2026-11-01',name:{fr:'Fête de la Révolution (1er novembre)',ar:'عيد الثورة (1 نوفمبر)',en:'Revolution Day (November 1st)'},isFixed:true,order:12}
    ];
    for(const holiday of holidays)if(!await this.getById('publicHolidays',holiday.id))await this.save('publicHolidays',{...holiday,isWorkable:true,notes:holiday.isFixed?'':'Date religieuse variable : à ajuster selon l’annonce officielle.'});
    ...
    // 304.2 Work rules & general terms (visible to every employee)
    if(!await this.getById('workRules','rules-current'))await this.save('workRules',{id:'rules-current',kind:'rules',version:1,titleI18n:{fr:'Règlement intérieur SARI SYSTÈME',ar:'النظام الداخلي SARI SYSTÈME',en:'SARI SYSTÈME Internal Work Rules'},contentI18n:{fr:'<h2>Règlement intérieur — SARI SYSTÈME</h2><p>Le présent règlement fixe les règles de conduite, d’hygiène, de sécurité et de discipline applicables à l’ensemble du personnel.</p><ul><li>Horaires de travail et pointage conformes à la configuration RH.</li><li>Respect des procédures qualité et de traçabilité des dispositifs médicaux.</li><li>Confidentialité absolue des données de l’entreprise et des clients.</li><li>Usage professionnel des ressources informatiques.</li></ul><p>Tout manquement expose son auteur aux sanctions prévues par la réglementation algérienne du travail (loi 90-11).</p>',ar:'<h2 dir="rtl">النظام الداخلي — SARI SYSTÈME</h2><p dir="rtl">يحدد هذا النظام قواعد السلوك والنظافة والسلامة والانضباط المطبقة على جميع الموظفين.</p><ul dir="rtl"><li>أوقات العمل والتسجيل وفق إعدادات الموارد البشرية.</li><li>احترام إجراءات الجودة وتتبع الأجهزة الطبية.</li><li>السرية التامة لبيانات الشركة والعملاء.</li><li>الاستخدام المهني للموارد المعلوماتية.</li></ul><p dir="rtl">كل مخالفة تعرّض صاحبها للعقوبات المنصوص عليها في قانون العمل الجزائري (90-11).</p>',en:'<h2>SARI SYSTÈME Internal Work Rules</h2><p>These rules set the conduct, hygiene, safety and discipline standards applying to all staff.</p><ul><li>Working hours and time tracking per the HR configuration.</li><li>Compliance with quality and traceability procedures for medical devices.</li><li>Strict confidentiality of company and client data.</li><li>Professional use of IT resources.</li></ul><p>Any breach exposes its author to the sanctions provided by Algerian labour law (90-11).</p>'},publishedAt:new Date().toISOString()});
    if(!await this.getById('workRules','terms-current'))await this.save('workRules',{id:'terms-current',kind:'terms',version:1,titleI18n:{fr:'Conditions générales & charte d’utilisation',ar:'الشروط العامة وميثاق الاستخدام',en:'General Terms & Usage Charter'},contentI18n:{fr:'<h2>Conditions générales d’utilisation des outils SARI</h2><p>L’accès à l’ERP, à la messagerie et aux documents est strictement personnel. Les identifiants ne doivent jamais être partagés. Les données consultées demeurent la propriété de SARI SYSTÈME.</p>',ar:'<h2 dir="rtl">الشروط العامة لاستخدام أدوات SARI</h2><p dir="rtl">الوصول إلى نظام ERP والمراسلة والوثائق شخصي تمامًا. لا يجوز مشاركة بيانات الدخول إطلاقًا. تظل البيانات المطّلَع عليها ملكًا لـ SARI SYSTÈME.</p>',en:'<h2>General terms of use for SARI tools</h2><p>Access to the ERP, messaging and documents is strictly personal. Credentials must never be shared. Viewed data remains the property of SARI SYSTÈME.</p>'},publishedAt:new Date().toISOString()});
