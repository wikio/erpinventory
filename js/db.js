...
class SariDB {
  constructor(dbName = 'SariSystemeDB', version = 5) {
    this.dbName = dbName;
      ...
      request.onupgradeneeded = (event) => {
        ...
        stores.forEach((storeName) => {
          if (!db.objectStoreNames.contains(storeName)) {
            ...
            } else if (storeName === 'careerRecords' || storeName === 'attendance' || storeName === 'performanceRecords') {
              store.createIndex('employeeId', 'employeeId', { unique: false });
  ...
  async exportJSON() {
    ...
    const stores = [
      ...
      'taxRecords', 'g50Payments', 'attendance', 'performanceRecords'
    ];
    const exportData = {
