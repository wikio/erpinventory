...
class SariDB {
  constructor(dbName = 'SariSystemeDB', version = 7) {
    this.dbName = dbName;
    ...
    this.initPromise = new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        const stores = [
          ...
          'importProfiles', 'apiTokens', 'apiEndpoints'
        ];
  ...
  async exportJSON() {
    ...
    const stores = [
      ...
      'productLots', 'stockMovements', 'inventoryCounts', 'importProfiles', 'apiTokens', 'apiEndpoints'
    ];
    const exportData = {
  ...
  async seedConfigurationData() {
    ...
    for(const [store,rows] of [['clientTypes',clientTypes],['supplierTypes',supplierTypes],['bankTypes',bankTypes],['countries',countries],['productCategories',productCategories],['salesStages',salesStages]]) for(const row of rows) await this.save(store,row);
    ...
    const roles=await this.getAll('roles');for(const role of roles){if(role.id==='admin')continue;role.permissions=[...new Set([...(role.permissions||[]),'masterData.view','bulkImport.view','api.view','inventoryOps.view'])];await this.save('roles',role);}
  }
