...
const DocumentSecurity = {
  ...
  async url(referenceCode, hash='') { const settings=await sariDB.getById('settings','app-settings')||{};const base=String(settings.verificationBaseUrl||'http://sari-systeme.com/code').replace(/\/$/,'');const verificationHash=hash||await this.hash(referenceCode);return `${base}/${encodeURIComponent(referenceCode)}-${verificationHash}`; },
