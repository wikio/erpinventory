...
const DocumentManager = {
  ...
  async upload(event) {
    ...
    const doc = { id: `doc-${crypto.randomUUID()}`, name: file.name, mimeType: file.type || 'application/octet-stream', size: file.size, data: await this.readFile(file), documentType: document.getElementById('ged-type').value, expirationDate: document.getElementById('ged-expiry').value, notes: document.getElementById('ged-notes').value.trim(), links: [{ recordType: this.context.recordType, recordId: this.context.recordId }], version: 1, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
  ...
  async editMetadata(id) {
    ...
    const type = prompt('Type de document :', doc.documentType || 'Autre'); if (type === null) return;
    const expiry = prompt('Date expiration (AAAA-MM-JJ, vide si aucune) :', doc.expirationDate || ''); if (expiry === null) return;
    const notes = prompt('Description / notes :', doc.notes || ''); if (notes === null) return;
    Object.assign(doc, { documentType: type.trim(), expirationDate: expiry.trim(), notes: notes.trim(), updatedAt: new Date().toISOString() });
  ...
  async unlink(id) {
    if (!confirm('Délier ce document de cet enregistrement ?')) return;
    const doc = await window.sariDB.getById('documents', id); if (!doc) return;
    doc.links = (doc.links || []).filter(link => !(link.recordType === this.context.recordType && link.recordId === this.context.recordId));
    if (doc.links.length) await window.sariDB.save('documents', doc); else if (confirm('Ce document ne sera plus lié. Le supprimer définitivement de la GED ?')) await window.sariDB.delete('documents', id); else await window.sariDB.save('documents', doc);
    await this.open(this.context.recordType, this.context.recordId, this.context.recordLabel);
  },
