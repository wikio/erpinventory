...
const DocumentManager = {
  ...
  async preview(id) {
    ...
    const url=await this.createObjectUrl(doc),win=window.open(url,'_blank','noopener,noreferrer');if(!win)app.showToast('Autorisez les fenêtres contextuelles pour afficher le document.','warning');setTimeout(()=>URL.revokeObjectURL(url),60000);
  },
