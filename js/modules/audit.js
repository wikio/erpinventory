...
const AuditModule = {
    ...
    container.innerHTML = `
      <div class="space-y-6">
        ...
        <div class="sari-tile p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p class="text-xs md:text-sm text-slate-600 dark:text-slate-300 mt-0.5">
              ${i18n.t('auditDescription','Journalisation inviolable des opérations utilisateurs, soumissions d’offres et mouvements de stock.')}
            </p>
          </div>
        ...
        <div class="sari-tile p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label class="block text-xs font-bold text-slate-600 dark:text-slate-300 mb-1">${i18n.t('searchAction','Rechercher une action')}</label>
            <input 
              type="text" 
          ...
          <div>
            <label class="block text-xs font-bold text-slate-600 dark:text-slate-300 mb-1">${i18n.t('filterErpModule','Filtrer par module ERP')}</label>
            <select 
              onchange="AuditModule.handleModuleFilter(this.value)"
              class="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded text-sm focus:outline-none focus:border-sari-blue"
            >
              <option value="all">${i18n.t('allModules','Tous les modules')}</option>
  ...
  async viewLog(id){const l=await sariDB.getById('auditLogs',id),root=document.getElementById('sari-modal-root');root.innerHTML=`<div class="fixed inset-0 z-50 sari-modal-backdrop flex items-center justify-center p-3"><article class="audit-consultation w-full max-w-5xl max-h-[92vh] overflow-y-auto"><header class="audit-consultation-header"><div><span class="sari-badge">${l.action}</span><h3 class="text-xl font-extrabold mt-2">${l.module} • ${l.affectedRecordId||i18n.t('system','Système')}</h3></div><div class="flex gap-2"><button onclick="DocumentManager.open('auditLog','${l.id}','${SariUtils.escapeHtml(l.action||l.id)}')" class="sari-btn px-4 bg-sari-blue text-white"><i data-lucide="paperclip"></i>${i18n.t('gedAttachments','Pièces GED')} (${this.gedCount(l.id)})</button><button onclick="app.closeModalRoot()">×</button></div></header><div class="audit-consultation-body"><div class="grid md:grid-cols-4 gap-3 my-4 text-xs"><div><span class="text-slate-400">${i18n.t('auditUser','Utilisateur')}</span><b class="block">${l.user} (${l.role})</b></div><div><span class="text-slate-400">${i18n.t('userId','ID utilisateur')}</span><b class="block">${l.actingUserId||'—'}</b></div><div><span class="text-slate-400">${i18n.t('timestamp','Horodatage')}</span><b class="block">${new Date(l.timestamp).toLocaleString()}</b></div><div><span class="text-slate-400">${i18n.t('typeId','Type / ID')}</span><b class="block">${l.recordType||l.module} / ${l.affectedRecordId||'—'}</b></div></div><p class="p-3 bg-slate-50 dark:bg-slate-800 rounded">${SariUtils.escapeHtml(l.description||'')}</p><h4 class="font-extrabold mt-5">${i18n.t('modifiedFields','Champs modifiés')} (${l.changes?.length||0})</h4><div class="overflow-x-auto"><table class="w-full text-xs mt-2"><thead><tr><th class="text-left p-2">${i18n.t('field','Champ')}</th><th>${i18n.t('before','Avant')}</th><th>${i18n.t('after','Après')}</th></tr></thead><tbody>${(l.changes||[]).map(x=>`<tr class="border-t"><td class="p-2 font-mono-tech text-sari-blue">${x.field}</td><td><pre class="whitespace-pre-wrap max-w-sm">${SariUtils.escapeHtml(JSON.stringify(x.before,null,2))}</pre></td><td><pre class="whitespace-pre-wrap max-w-sm">${SariUtils.escapeHtml(JSON.stringify(x.after,null,2))}</pre></td></tr>`).join('')||`<tr><td colspan="3" class="p-4 text-slate-400">${i18n.t('legacyAuditNoDiff','Ancienne entrée sans différence structurée.')}</td></tr>`}</tbody></table></div></div></article></div>`;window.SariIcons?.hydrate();},
  async editLog(id){if(auth.currentRole!=='admin')return;const l=await sariDB.getById('auditLogs',id),v=await DialogManager.form(i18n.t('editAuditEntry','Modifier une entrée d’audit'),[{name:'action',label:'Action',value:l.action},{name:'module',label:'Module',value:l.module},{name:'description',label:'Description',type:'textarea',value:l.description}]);if(!v)return;const before=JSON.stringify(l);Object.assign(l,v,{editedAt:new Date().toISOString(),editedBy:auth.currentUser.name});await sariDB.save('auditLogs',l);await sariDB.save('auditLogs',{id:`audit-${crypto.randomUUID()}`,user:auth.currentUser.name,role:auth.currentRole,action:'EDIT_AUDIT_LOG',module:'Audit',description:`Modification de ${id}. Empreinte précédente: ${before.slice(0,180)}`,timestamp:new Date().toISOString()});this.render();},
