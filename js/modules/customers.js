        ...
        <div class="sari-tile w-full max-w-5xl max-h-[94vh] overflow-y-auto bg-white dark:bg-slate-900 p-6 shadow-2xl relative">
          ...
          <form onsubmit="CustomersModule.saveCustomer(event)" class="space-y-4 text-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label class="block text-xs font-bold text-slate-600 dark:text-slate-300 mb-1">Type de Client *</label>
                <input id="cust-type" list="client-type-options" value="${cust.type||''}" class="w-full px-3 py-2 border rounded bg-white dark:bg-slate-800 font-semibold" required autocomplete="off"><datalist id="client-type-options">${(this.state.clientTypes||[]).filter(x=>x.isActive).map(x=>`<option value="${x.id}">${x.name?.[i18n.currentLang]||x.name?.fr}</option>`).join('')}</datalist>
              </div>
              <div><label class="block text-xs font-bold text-slate-600 dark:text-slate-300 mb-1">Pays *</label><input id="cust-country" list="customer-country-options" value="${cust.countryCode||'DZA'}" class="w-full px-3 py-2 border rounded bg-white dark:bg-slate-800" required autocomplete="off"><datalist id="customer-country-options">${this.state.countries.filter(c=>c.isActive).map(c=>`<option value="${c.iso3}">${c.name?.[i18n.currentLang]||c.name?.fr}</option>`).join('')}</datalist></div>
              <div>
                <label class="block text-xs font-bold text-slate-600 dark:text-slate-300 mb-1">Wilaya (Algérie) *</label>
