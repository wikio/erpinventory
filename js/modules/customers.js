...
const CustomersModule = {
      ...
      <div class="fixed inset-0 z-50 flex items-center justify-center p-4 sari-modal-backdrop">
        <div class="sari-tile w-full max-w-5xl max-h-[94vh] overflow-y-auto bg-white dark:bg-slate-900 p-6 shadow-2xl relative">
          ...
          <form onsubmit="CustomersModule.saveCustomer(event)" class="space-y-4 text-sm">
            ...
            <div class="grid md:grid-cols-[180px_1fr] gap-4"><label class="doc-label">Logo client${cust.logo?`<img src="${cust.logo}" class="h-16 object-contain my-2">`:''}<input id="cust-logo" type="file" accept="image/*" class="doc-input"></label>${RichTextEditor.html('cust-rich-details',cust.richDetails||'','Informations détaillées / conditions spéciales')}</div>
  ...
  async saveCustomer(e) {
    ...
    const payload = {
      ...
      logo: document.getElementById('cust-logo').files[0] ? await SariUtils.fileToDataURL(document.getElementById('cust-logo').files[0]) : original.logo || '',
  ...
  async open360(id){const c=this.state.customers.find(x=>x.id===id),docs=this.state.orders.filter(o=>o.customerId===id),total=docs.reduce((s,o)=>s+Number(o.total||0),0),outstanding=docs.filter(o=>!['paid','closed'].includes(o.status)).reduce((s,o)=>s+Number(o.total||0),0);await DialogManager.form(`Client 360° • ${c.name}`,[{name:'statistics',label:'Statistiques',type:'textarea',value:`Volume vendu: ${i18n.formatCurrency(total)}\nSolde en attente: ${i18n.formatCurrency(outstanding)}\nCommandes/documents: ${docs.length}\nPanier moyen: ${i18n.formatCurrency(docs.length?total/docs.length:0)}`},{name:'documents',label:'Chaîne documentaire',type:'textarea',value:docs.map(o=>`${o.referenceCode||o.id} • ${o.documentType||'commande'} • ${o.status}`).join('\n')||'Aucun document'}],{confirmText:'Fermer'});},
