        ...
        <div class="sari-tile w-full max-w-6xl max-h-[94vh] overflow-y-auto bg-white dark:bg-slate-900 p-6 shadow-2xl relative">
          ...
          <form onsubmit="ImportExportModule.saveShipment(event)" class="space-y-4 text-sm">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div><label class="block text-xs font-bold text-slate-600 dark:text-slate-300 mb-1">Pays partenaire *</label><input id="sh-country" list="shipment-country-options" value="${sh.partnerCountryCode||'DZA'}" class="w-full px-3 py-2 border rounded bg-white dark:bg-slate-800" required autocomplete="off"><datalist id="shipment-country-options">${this.state.countries.map(c=>`<option value="${c.iso3}">${c.name?.[i18n.currentLang]||c.name?.fr} (${c.iso3})</option>`).join('')}</datalist></div>
              <div>
