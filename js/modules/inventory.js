          ...
          <form onsubmit="InventoryModule.saveProduct(event)" class="space-y-4 text-sm">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input id="form-category" list="product-category-options" value="${prod.category}" class="w-full px-3 py-2 border rounded bg-white dark:bg-slate-800" required autocomplete="off"><datalist id="product-category-options">${(this.state.productCategories||[]).map(c=>`<option value="${c.id}">${c.name?.[i18n.currentLang]||c.name?.fr} • PRO${c.code}</option>`).join('')}</datalist>
              </div>
            </div>
