...
const SalesModule = {
  state: {
    ...
    referralId: '',
    couponCode: '',
    purchaseDocuments: [], tenders: [], bankAccounts: [], referrals: [], coupons: [], paymentMethods: [], salesStages: [], historyPage:1, historyPageSize:10, historyStatus:'all', historyType:'all', historyCustomer:'all', historyFrom:'', historyTo:''
  },
  ...
  async render(containerId = 'sari-main-view') {
    ...
    [this.state.purchaseDocuments,this.state.tenders,this.state.bankAccounts,this.state.referrals,this.state.coupons,this.state.paymentMethods,this.state.salesStages] = await Promise.all(['purchaseDocuments','tenders','bankAccounts','referrals','coupons','paymentMethods','salesStages'].map(s=>sariDB.getAll(s)));
  ...
  applyCoupon() {
    ...
    if(!coupon)return app.showToast('Coupon invalide ou expiré.','warning');
  ...
  async openPrintModal(orderId, docType = 'facture') {
    ...
    const configuredCode = ({ invoice:'FAV', quote:'DVV', purchase_order:'BCA', delivery_note:'LIV' })[requestedType];
    o.referenceCodes ||= {};
        ...
        <div id="print-doc-area" class="sari-tile w-full max-w-3xl bg-white dark:bg-slate-900 p-8 shadow-2xl relative max-h-[95vh] overflow-y-auto text-slate-900">
          ...
          <div class="${template.elements?.length ? 'hidden' : ''} bg-white text-slate-900 p-4 border border-slate-300" style="border-top:8px solid ${template.accent}">
            ...
            <div class="grid ${hideAmounts ? 'grid-cols-1' : 'grid-cols-2'} gap-8 items-start">
              <div><p class="text-xs text-slate-600"><strong>Mode de Règlement :</strong> ${o.paymentMethod || 'Virement Bancaire'}<br/><strong>Compte Bancaire SARI :</strong> BNA Agence 001 - RIB 00100161609876543209</p>${o.documentNote?`<div class="mt-3 p-2 border rounded text-xs"><strong>Note :</strong> ${SariUtils.escapeHtml(o.documentNote)}</div>`:''}<div class="flex items-end gap-4 mt-4"><div><canvas id="commercial-qr" width="88" height="88" class="border"></canvas><p class="text-[8px] text-slate-400 max-w-28 break-all">${SariUtils.escapeHtml(printVerificationUrl)}</p><canvas id="commercial-barcode" width="180" height="55" class="mt-2 border"></canvas></div><div class="pt-2 border-t border-slate-300 text-center w-48"><p class="text-xs font-bold">Cachet & Signature</p><div class="h-10 text-slate-300 text-xs italic pt-3">(Approuvé & Certifié)</div></div></div></div>
