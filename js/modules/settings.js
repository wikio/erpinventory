...
const SettingsModule = {
  ...
  async render(containerId = 'sari-main-view') {
    ...
    [this.state.checklistTemplates, this.state.roles, this.state.documentTemplates, this.state.vatRates, this.state.documentCodes, this.state.paymentMethods, this.state.banks, this.state.bankAccounts, this.state.coupons, this.state.referrals, this.state.bankTypes] = await Promise.all([
      'checklistTemplates','roles','documentTemplates','vatRates','documentCodes','paymentMethods','banks','bankAccounts','coupons','referrals','bankTypes'
    ...
    return `
      <div class="sari-tile p-6 space-y-6">
        <div class="border-b border-slate-200 dark:border-slate-800 pb-3">
          ...
          <p class="text-xs text-slate-500 mt-0.5">
            Exportez l'ensemble des tables en CSV pour vos tableurs Excel, effectuez un backup JSON de vos 53 stores ou restaurez votre système.
          </p>
        </div>
