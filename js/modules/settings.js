...
const SettingsModule = {
    ...
    return `
      <div class="sari-tile p-6 space-y-6">
        <div class="border-b border-slate-200 dark:border-slate-800 pb-3">
          ...
          <p class="text-xs text-slate-500 mt-0.5">
            Exportez l'ensemble des tables en CSV pour vos tableurs Excel, effectuez un backup JSON de vos 41 stores ou restaurez votre système.
          </p>
        </div>
  ...
  async editBankAccount(id=''){const old=this.state.bankAccounts.find(a=>a.id===id)||{},bank=this.state.banks.find(b=>b.id===old.bankId)||{};const v=await DialogManager.form(id?'Modifier le compte':'Nouveau compte bancaire',[{name:'name',label:'Nom du compte',value:old.name||'',required:true},{name:'bank',label:'Banque',value:bank.name||'',required:true},{name:'iban',label:'RIB / IBAN',value:old.iban||'',required:true},{name:'currency',label:'Devise',type:'select',value:old.currency||'DZD',options:['DZD','EUR','USD']},{name:'accountType',label:'Type de compte',type:'select',value:old.accountType||'bank',options:['bank','cash','online']},{name:'paymentMethodId',label:'Mode de paiement lié',type:'select',value:old.paymentMethodId||'',options:this.state.paymentMethods.map(m=>({value:m.id,label:m.label?.fr||m.code}))},{name:'country',label:'Pays',value:old.country||'Algérie'},{name:'address',label:'Adresse',value:old.address||''},{name:'phone',label:'Téléphone',value:old.phone||''},{name:'contactPerson',label:'Contact',value:old.contactPerson||''},{name:'details',label:'Détails',type:'textarea',value:old.details||''},{name:'logo',label:'Logo',type:'image',value:old.logo||''}]);if(!v)return;const bankId=old.bankId||`bank-${crypto.randomUUID()}`;await sariDB.save('banks',{...bank,id:bankId,name:v.bank,isActive:true});const logo=v.logo||old.logo||'';await sariDB.save('bankAccounts',{...old,id:id||`account-${crypto.randomUUID()}`,bankId,name:v.name,iban:v.iban,currency:v.currency,accountType:v.accountType,paymentMethodId:v.paymentMethodId,country:v.country,address:v.address,phone:v.phone,contactPerson:v.contactPerson,details:RichTextEditor.sanitize(v.details),logo,isActive:old.isActive??true});this.render();},
  async viewBankAccount(id){const a=this.state.bankAccounts.find(x=>x.id===id),all=[...(await sariDB.getAll('orders')),...(await sariDB.getAll('purchaseDocuments'))],docs=all.filter(d=>d.bankAccountId===id);const v=await DialogManager.form(`Compte • ${a.name}`,[{name:'stats',label:'Statistiques',type:'textarea',value:`Documents: ${docs.length}\nVolume: ${i18n.formatCurrency(docs.reduce((s,d)=>s+Number(d.total||0),0))}\nIBAN: ${a.iban}\n${a.address||''} ${a.phone||''}`},{name:'action',label:'Action',type:'select',options:[{value:'close',label:'Fermer'},{value:'ged',label:'Ouvrir la GED'}]}],{confirmText:'Continuer'});if(v?.action==='ged')DocumentManager.open('bankAccount',id,a.name);},
