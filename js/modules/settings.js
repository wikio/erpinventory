...
const SettingsModule = {
  ...
  getTabHtml(tab, s, canWrite) {
    ...
    if (tab === 'database') {
      ...
      return `<div class="space-y-5"><section class="sari-tile p-6"><div class="flex justify-between border-b pb-4"><div><span class="sari-badge ${db.active?'bg-green-100 text-green-700':'bg-slate-100 text-slate-600'}">${db.active?'BACKEND EXTERNE ACTIF':'INDEXEDDB OFFLINE'}</span><h3 class="font-extrabold text-lg mt-2">Connexion base de données externe</h3><p class="text-xs text-slate-500">Test réel côté serveur pour MySQL, PostgreSQL et MongoDB. Les secrets saisis restent uniquement en mémoire serveur, sauf SARI_DB_PASSWORD fourni par l’environnement.</p></div><i data-lucide="database-zap" class="w-10 h-10 text-sari-blue"></i></div><form id="external-db-form" onsubmit="SettingsModule.saveExternalDB(event)" class="grid md:grid-cols-3 gap-4 mt-5"><label class="doc-label">Type de base<select id="ext-db-type" class="doc-input" onchange="SettingsModule.externalTypeChanged(this.value)"><option value="mysql" ${targetType==='mysql'?'selected':''}>MySQL</option><option value="postgresql" ${targetType==='postgresql'?'selected':''}>PostgreSQL</option><option value="mongodb" ${targetType==='mongodb'?'selected':''}>MongoDB</option></select></label><label class="doc-label">Hôte<input id="ext-db-host" value="${SariUtils.escapeHtml(db.host||'localhost')}" class="doc-input" required></label><label class="doc-label">Port<input id="ext-db-port" type="number" value="${db.port||({mysql:3306,postgresql:5432,mongodb:27017}[targetType])}" class="doc-input" required></label><label class="doc-label">Base de données<input id="ext-db-name" value="${SariUtils.escapeHtml(db.database||'sari_erp_prod')}" class="doc-input" required></label><label class="doc-label">Nom utilisateur<input id="ext-db-user" autocomplete="username" value="${SariUtils.escapeHtml(db.username||'')}" class="doc-input"></label><label class="doc-label">Mot de passe<input id="ext-db-password" type="password" autocomplete="new-password" placeholder="${db.hasPassword?'Secret déjà disponible — laisser vide pour le conserver':'Mot de passe de connexion'}" class="doc-input"><small class="field-helper">Jamais écrit dans .runtime. Une saisie UI reste disponible en mémoire jusqu’au redémarrage serveur.</small></label><label class="doc-label">SSL<select id="ext-db-ssl" class="doc-input"><option value="false" ${!db.ssl?'selected':''}>Désactivé</option><option value="true" ${db.ssl?'selected':''}>Activé</option></select></label><label class="doc-label">Backend actif<select id="ext-db-active" class="doc-input"><option value="false" ${!db.active?'selected':''}>Non — IndexedDB uniquement</option><option value="true" ${db.active?'selected':''}>Oui — synchroniser via API</option></select></label><div class="flex items-end gap-2"><button type="button" onclick="SettingsModule.testExternalDB()" class="sari-btn px-4 py-2 bg-sari-lime text-slate-900">Tester réellement</button><button type="submit" class="sari-btn px-4 py-2 bg-sari-blue text-white">Tester & enregistrer</button></div></form><div id="external-db-feedback" class="mt-4"></div></section>
      <section class="sari-tile p-6"><div class="flex flex-col md:flex-row justify-between gap-3"><div><h3 class="font-extrabold text-lg">Migration IndexedDB → <span id="external-migration-target">${targetLabel}</span></h3><p class="text-xs text-slate-500">Transfert idempotent par table/collection, vérification des écritures dans la cible et rapport détaillé.</p></div><div class="flex gap-2"><button onclick="SettingsModule.preflightExternalMigration()" ${db.active?'':'disabled'} class="sari-btn px-4 bg-slate-200 disabled:opacity-40">Prévalider</button><button id="external-migration-start" onclick="SettingsModule.migrateToExternalDB()" ${db.active?'':'disabled'} class="sari-btn px-5 bg-sari-amber text-slate-900 disabled:opacity-40">Lancer la migration</button></div></div><div class="external-progress-track mt-4"><div id="external-migration-progress" class="external-progress-bar" style="width:0%"></div></div><div class="flex justify-between mt-1 text-xs"><span id="external-migration-step">En attente</span><b id="external-migration-percent">0%</b></div><div id="external-migration-results" class="mt-4 grid md:grid-cols-3 gap-2"></div></section>
      <section class="sari-tile p-5"><header class="flex justify-between gap-3"><div><h3 class="font-extrabold flex items-center gap-2"><i data-lucide="terminal" class="w-4 h-4 text-sari-blue"></i>Console diagnostique</h3><p class="text-xs text-slate-500">Tentatives de connexion et étapes de migration en temps réel.</p></div><button onclick="SettingsModule.clearDbDiagnostics()" class="doc-action">Effacer</button></header><div id="external-db-console" class="external-db-console mt-4"><p class="text-slate-500">Aucun événement.</p></div></section><section class="sari-tile p-4 text-xs"><b>Architecture offline-first :</b> IndexedDB reste le cache local et la file de synchronisation. La cible active reçoit ensuite les mutations via le connecteur serveur.</section></div>`;
    }
  ...
  externalTypeChanged(type){const ports={mysql:3306,postgresql:5432,mongodb:27017},labels={mysql:'MySQL',postgresql:'PostgreSQL',mongodb:'MongoDB'};document.getElementById('ext-db-port').value=ports[type];const target=document.getElementById('external-migration-target'),button=document.getElementById('external-migration-start'),step=document.getElementById('external-migration-step');if(target)target.textContent=labels[type];if(button)button.disabled=true;if(step)step.textContent='Enregistrez la nouvelle cible avant de migrer.';},
  showExternalFeedback(result,success){const el=document.getElementById('external-db-feedback');if(el)el.innerHTML=`<div class="p-3 rounded border ${success?'bg-green-50 border-green-300 text-green-800':'bg-red-50 border-red-300 text-red-700'}"><b>${success?'✓ Connexion réussie':'✗ Échec de connexion'}</b><p class="text-xs">${SariUtils.escapeHtml(result.message||result.error||'')}</p>${result.code?`<code class="block text-[10px] mt-1">${SariUtils.escapeHtml(result.code)} • ${SariUtils.escapeHtml(result.target||'')}</code>`:''}${result.hint?`<small class="block mt-1">${SariUtils.escapeHtml(result.hint)}</small>`:''}${result.serverVersion?`<small class="block mt-1">Serveur: ${SariUtils.escapeHtml(result.serverVersion)}</small>`:''}${result.latencyMs!==undefined?`<span class="sari-badge mt-1">${result.latencyMs} ms</span>`:''}</div>`;},
  async testExternalDB(){const payload=this.externalPayload(),feedback=document.getElementById('external-db-feedback');if(feedback)feedback.innerHTML='<div class="p-3 border rounded flex gap-2"><i data-lucide="loader-2" class="animate-spin"></i>Connexion réelle en cours…</div>';window.SariIcons?.hydrate();try{const response=await fetch('/api/db/test',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}),result=await response.json();this.showExternalFeedback(result,response.ok&&result.success);return response.ok&&result.success;}catch(error){this.showExternalFeedback({error:error.message,code:'NETWORK_ERROR'},false);return false;}finally{await this.refreshDbDiagnostics();}},
  async saveExternalDB(event){event.preventDefault();const payload=this.externalPayload();try{const response=await fetch('/api/db/config',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}),result=await response.json();this.showExternalFeedback(result,response.ok);if(response.ok){this.state.externalDbConfig=result;await dbAdapter.setDriver(result.active?result.type:'indexeddb',{serverManaged:true});app.showToast(`Connexion ${String(result.type).toUpperCase()} enregistrée côté serveur.`,'success');await this.render();}return response.ok;}catch(error){this.showExternalFeedback({error:error.message,code:'NETWORK_ERROR'},false);return false;}finally{await this.refreshDbDiagnostics();}},
