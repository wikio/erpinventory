...
const SettingsModule = {
  ...
  async testExternalDB(){const payload=this.externalPayload(),feedback=document.getElementById('external-db-feedback'),controller=new AbortController(),timeoutMs=15000,timer=setTimeout(()=>controller.abort('connection-test-timeout'),timeoutMs);if(feedback)feedback.innerHTML=`<div class="p-3 border rounded flex gap-2"><i data-lucide="loader-2" class="animate-spin"></i>Connexion réelle en cours… <span class="text-slate-400">(maximum ${timeoutMs/1000}s)</span></div>`;window.SariIcons?.hydrate();try{const response=await fetch('/api/db/test',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload),signal:controller.signal});let result;try{result=await response.json();}catch(_){result={success:false,error:`Réponse serveur invalide (HTTP ${response.status}).`,code:'INVALID_RESPONSE'};}this.showExternalFeedback(result,response.ok&&result.success);return response.ok&&result.success;}catch(error){const timedOut=error?.name==='AbortError'||controller.signal.aborted;this.showExternalFeedback({error:timedOut?`Le test a été arrêté après ${timeoutMs/1000} secondes. Vérifiez l’hôte, le port, le pare-feu et redémarrez le serveur Node après une mise à jour.`:error.message,code:timedOut?'CLIENT_TIMEOUT':'NETWORK_ERROR'},false);return false;}finally{clearTimeout(timer);await Promise.race([this.refreshDbDiagnostics(),new Promise(resolve=>setTimeout(resolve,2000))]);}},
