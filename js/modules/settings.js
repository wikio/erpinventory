...
const SettingsModule = {
  ...
  getTabHtml(tab, s, canWrite) {
    ...
    if (tab === 'database') {
      ...
      return `<div class="space-y-5"><section class="sari-tile p-6"><div class="flex justify-between border-b pb-4"><div><span class="sari-badge ${db.active?'bg-green-100 text-green-700':'bg-slate-100 text-slate-600'}">${db.active?'BACKEND EXTERNE ACTIF':'INDEXEDDB OFFLINE'}</span><h3 class="font-extrabold text-lg mt-2">Connexion base de données externe</h3><p class="text-xs text-slate-500">Le navigateur communique uniquement avec le serveur SARI. Le mot de passe est fourni par la variable serveur SARI_DB_PASSWORD et n’est jamais enregistré dans un fichier ni dans IndexedDB.</p></div><i data-lucide="database-zap" class="w-10 h-10 text-sari-blue"></i></div><form id="external-db-form" onsubmit="SettingsModule.saveExternalDB(event)" class="grid md:grid-cols-3 gap-4 mt-5"><label class="doc-label">Type de base<select id="ext-db-type" class="doc-input" onchange="SettingsModule.externalTypeChanged(this.value)"><option value="mysql" ${db.type==='mysql'?'selected':''}>MySQL</option><option value="postgresql" ${db.type==='postgresql'?'selected':''}>PostgreSQL</option><option value="mongodb" ${db.type==='mongodb'?'selected':''}>MongoDB</option></select></label><label class="doc-label">Hôte<input id="ext-db-host" value="${SariUtils.escapeHtml(db.host||'localhost')}" class="doc-input" required></label><label class="doc-label">Port<input id="ext-db-port" type="number" value="${db.port||({mysql:3306,postgresql:5432,mongodb:27017}[db.type]||3306)}" class="doc-input" required></label><label class="doc-label">Base de données<input id="ext-db-name" value="${SariUtils.escapeHtml(db.database||'sari_erp_prod')}" class="doc-input" required></label><label class="doc-label">Nom utilisateur<input id="ext-db-user" autocomplete="username" value="${SariUtils.escapeHtml(db.username||'')}" class="doc-input"></label><label class="doc-label">Mot de passe<input id="ext-db-password" type="password" autocomplete="new-password" placeholder="Test ponctuel uniquement" class="doc-input"><small class="field-helper">Utilisé uniquement pour le test. Définissez SARI_DB_PASSWORD sur le serveur pour activer la connexion.</small></label><label class="doc-label">SSL<select id="ext-db-ssl" class="doc-input"><option value="false">Désactivé</option><option value="true">Activé</option></select></label><label class="doc-label">Backend actif<select id="ext-db-active" class="doc-input"><option value="false" ${!db.active?'selected':''}>Non — IndexedDB uniquement</option><option value="true" ${db.active?'selected':''}>Oui — synchroniser via API</option></select></label><div class="flex items-end gap-2"><button type="button" onclick="SettingsModule.testExternalDB()" class="sari-btn px-4 py-2 bg-sari-lime text-slate-900">Tester</button><button type="submit" class="sari-btn px-4 py-2 bg-sari-blue text-white">Tester & enregistrer</button></div></form><div id="external-db-feedback" class="mt-4"></div></section><section class="sari-tile p-6"><div class="flex justify-between"><div><h3 class="font-extrabold text-lg">Migration IndexedDB → ${String(db.type||'externe').toUpperCase()}</h3><p class="text-xs text-slate-500">Transfert transactionnel et idempotent vers une table normalisée par entité. Les échecs sont détaillés par enregistrement.</p></div><button onclick="SettingsModule.migrateToExternalDB()" ${db.active?'':'disabled'} class="sari-btn px-5 bg-sari-amber text-slate-900 disabled:opacity-40">Lancer la migration</button></div><div class="w-full h-3 bg-slate-200 rounded-full overflow-hidden mt-4"><div id="external-migration-progress" class="h-full bg-sari-blue transition-all" style="width:0%"></div></div><div id="external-migration-results" class="mt-4 grid md:grid-cols-3 gap-2"></div></section><section class="sari-tile p-4 text-xs"><b>Architecture offline-first :</b> IndexedDB reste le cache local et la file de synchronisation. Sans configuration externe, l’application reste entièrement fonctionnelle hors ligne.</section></div>`;
    }
