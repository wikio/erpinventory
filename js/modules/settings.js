...
const SettingsModule = {
    ...
    container.innerHTML = `
      <div class="space-y-6">
        ...
        <div class="sari-tile p-2 flex flex-wrap gap-2 border-b-2 border-sari-blue">
          ...
          <button onclick="SettingsModule.openSmtpSettings()" class="px-4 py-2 rounded text-xs font-bold transition flex items-center gap-1.5 hover:bg-slate-100 dark:hover:bg-slate-800"><i data-lucide="mail" class="w-4 h-4"></i> ${i18n.t('smtpConfiguration','Configuration SMTP')}</button>
  ...
  getTabHtml(tab, s, canWrite) {
    ...
    if (tab === 'permissions') {
      const modules=['dashboard','inventory','tenders','importExport','suppliers','sales','customers','reports','documents','hr','payslips','leaves','contracts','occasionalWorkers','tasks','portal','messages','purchases','ged','taxes','masterData','inventoryOps','bulkImport','api','users','vatRates','documentCodes','settings']; const actions=['view','create','edit','delete'];
