...
const TendersModule = {
  ...
  async openDocumentChain(tenderId){const orders=(await sariDB.getAll('orders')).filter(o=>o.linkedTenderId===tenderId),purchases=(await sariDB.getAll('purchaseDocuments')).filter(d=>d.linkedTenderId===tenderId),links=(await sariDB.getAll('documentLinks')).filter(l=>l.targetId===tenderId||l.sourceId===tenderId);await DialogManager.form('Traçabilité documentaire de la consultation',[{name:'chain',label:'Chaîne achat ↔ vente ↔ consultation',type:'textarea',value:[...purchases.map(d=>`ACHAT ${d.referenceCode} • ${d.status}`),...orders.map(o=>`VENTE ${o.referenceCode||o.id} • ${o.status}`),...links.map(l=>`LIEN ${l.sourceType}:${l.sourceId} → ${l.targetType}:${l.targetId}`)].join('\n')||'Aucun document lié'}],{confirmText:'Fermer'});},
