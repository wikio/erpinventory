...
const TendersModule = {
  ...
  async editChecklistItem(id) { const item=await sariDB.getById('checklistItems',id);if(!item)return;const label=prompt('Libellé :',item.label);if(label===null)return;const due=prompt('Échéance (AAAA-MM-JJ) :',item.dueDate||'');if(due===null)return;const notes=prompt('Notes :',item.notes||'');if(notes===null)return;Object.assign(item,{label:label.trim(),dueDate:due.trim(),notes:notes.trim(),updatedAt:new Date().toISOString()});await sariDB.save('checklistItems',item);await this.openDocsModal(item.tenderId); },
  async deleteChecklistItem(id) { const item=await sariDB.getById('checklistItems',id);if(item&&confirm('Supprimer cet élément de checklist ?')){await sariDB.delete('checklistItems',id);await this.openDocsModal(item.tenderId);} },
