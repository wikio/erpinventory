...
const RichTextEditor = {
  html(id, value = '', label = 'Description détaillée') {
    return `<div class="rich-editor-wrap"><label class="doc-label">${label}</label><div class="rich-editor-toolbar" role="toolbar" aria-label="Mise en forme"><button type="button" onclick="RichTextEditor.command('${id}','bold')"><b>B</b></button><button type="button" onclick="RichTextEditor.command('${id}','italic')"><i>I</i></button><button type="button" onclick="RichTextEditor.command('${id}','underline')"><u>U</u></button><button type="button" onclick="RichTextEditor.command('${id}','insertUnorderedList')">• Liste</button><button type="button" onclick="RichTextEditor.command('${id}','formatBlock','h3')">Titre</button><button type="button" onclick="RichTextEditor.command('${id}','createLink',prompt('URL du lien :'))">Lien</button></div><div id="${id}" class="rich-editor" contenteditable="true" role="textbox" aria-multiline="true">${this.sanitize(value)}</div></div>`;
  },
