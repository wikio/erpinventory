...
async function handleApi(req, res, pathname) {
  ...
  if (pathname === '/api/auth/change-password' && req.method === 'POST') { const session=getSession(req);if(!session)return json(res,401,{error:'Authentication required',code:'AUTH_REQUIRED'});try{const body=await readJson(req);if(body.newPassword!==body.confirmation)return json(res,400,{error:'Password confirmation does not match',code:'PASSWORD_MISMATCH'});const user=authVault.changePassword(session.session.user.id,body.currentPassword,body.newPassword);session.session.user=authVault.find(user.id);for(const[token,item]of sessions)if(token!==session.token&&item.user?.id===user.id)sessions.delete(token);return json(res,200,{success:true,user});}catch(error){return json(res,400,{error:error.message,code:error.code||'PASSWORD_CHANGE_FAILED'});} }
