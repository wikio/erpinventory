...
CREATE TABLE `banks` (`id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY, `name` VARCHAR(180) NOT NULL, `swift` VARCHAR(32), `country` VARCHAR(3), `is_active` BOOLEAN DEFAULT TRUE);
CREATE TABLE `bank_accounts` (`id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY, `bank_id` BIGINT UNSIGNED NOT NULL, `name` VARCHAR(180) NOT NULL, `iban` VARCHAR(100), `currency` VARCHAR(10) DEFAULT 'DZD', `is_default` BOOLEAN DEFAULT FALSE, `is_active` BOOLEAN DEFAULT TRUE, FOREIGN KEY (`bank_id`) REFERENCES `banks` (`id`));
