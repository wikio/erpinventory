...
export function calculatePayslip(input: PayslipAmounts) {
  const gross=['baseSalary','seniorityAllowance','performanceBonus','otherBonuses','transportAllowance','housingAllowance','mealAllowance','overtimeAmount','otherAllowances'].reduce((sum,key)=>sum+amount(input[key as keyof PayslipAmounts]),0);
  ...
  const other=amount(input.advances)+amount(input.loans)+amount(input.otherDeductions),deductions=cnasEmployee+irg+other;
