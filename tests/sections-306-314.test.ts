...
describe('section 307 — Algerian weekend (Friday/Saturday) and Sunday-first calendar', () => {
  ...
  it('orders calendar columns Sunday-first with configured weekend days always last', () => {
    ...
    // Section 322 fix: the order must remain a chronological rotation (Sat then Sun).
expect(weekColumnOrder({ ...defaultSchedule, workingDays: [1, 2, 3, 4, 5] })).toEqual([1, 2, 3, 4, 5, 6, 0]);
    // Scattered weekend days: the engine picks the best chronological rotation.
expect(weekColumnOrder({ ...defaultSchedule, workingDays: [0, 2, 4, 6] })).toEqual([2, 3, 4, 5, 6, 0, 1]);
  });
